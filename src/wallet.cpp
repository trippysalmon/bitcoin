...
CAmount CWallet::GetUnconfirmedBalance() const
{
    CAmount nTotal = 0;
    {
        ...
        {
            const CWalletTx* pcoin = &(*it).second;
            if (!IsFinalTx(*pcoin, GetHeight()) || (!pcoin->IsTrusted() && pcoin->GetDepthInMainChain() == 0))
                nTotal += pcoin->GetAvailableCredit();
        }
    }
    return nTotal;
        ...
        LOCK2(cs_main, cs_wallet);
        {
            ...
            while (true)
            {
                ...
                *static_cast<CTransaction*>(&wtxNew) = CTransaction(txNew);
                ...
                if (!Policy().ValidateFee(nFeeNeeded, nBytes))
                {
...
CAmount CWallet::GetMinimumFee(unsigned int nTxBytes, unsigned int nConfirmTarget)
{
    ...
    if (!Policy().ValidateFee(nFeeNeeded, nTxBytes))
...
std::map<CTxDestination, CAmount> CWallet::GetAddressBalances()
{
...

    {
        ...
        BOOST_FOREACH(PAIRTYPE(uint256, CWalletTx) walletEntry, mapWallet)
        {
            ...
            if (!IsFinalTx(*pcoin, GetHeight()) || !pcoin->IsTrusted())
                continue;
