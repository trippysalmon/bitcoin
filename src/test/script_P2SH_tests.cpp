...
BOOST_AUTO_TEST_CASE(sign)
{
    ...
    BOOST_CHECK(Policy().CheckTxPreInputs(txFrom, reason));
...
BOOST_AUTO_TEST_CASE(set)
{
    ...
    {
        BOOST_CHECK_MESSAGE(SignSignature(keystore, txFrom, txTo[i], 0), strprintf("SignSignature %d", i));
        BOOST_CHECK_MESSAGE(Policy().CheckTxPreInputs(txTo[i], reason), strprintf("txTo[%d].IsStandard", i));
    }
}
...
BOOST_AUTO_TEST_CASE(AreInputsStandard)
{
    ...
    BOOST_CHECK(Policy().CheckTxWithInputs(txTo, coins));
    ...
    BOOST_CHECK(!Policy().CheckTxWithInputs(txToNonStd1, coins));
    ...
    BOOST_CHECK(!Policy().CheckTxWithInputs(txToNonStd2, coins));
