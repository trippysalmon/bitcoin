...
{
    Container<CPolicy> cTestPolicy(Policy::Factory(Policy::STANDARD));
    LOCK(cs_main);
    ...
    BOOST_CHECK(cTestPolicy.Get().ApproveTx(txFrom, reason));
...
BOOST_AUTO_TEST_CASE(set)
{
    ...
    {
        BOOST_CHECK_MESSAGE(SignSignature(keystore, txFrom, txTo[i], 0), strprintf("SignSignature %d", i));
        BOOST_CHECK_MESSAGE(cTestPolicy.Get().ApproveTx(txTo[i], reason), strprintf("txTo[%d].IsStandard", i));
    }
}
...
BOOST_AUTO_TEST_CASE(AreInputsStandard)
{
    ...
    BOOST_CHECK(cTestPolicy.Get().ApproveTxInputs(txTo, coins));
    ...
    BOOST_CHECK(!cTestPolicy.Get().ApproveTxInputs(txToNonStd1, coins));
    ...
    BOOST_CHECK(!cTestPolicy.Get().ApproveTxInputs(txToNonStd2, coins));
