 ...
 */
static int64_t GetSpendHeight(const CCoinsViewCache& inputs)
{
    LOCK(cs_main);
...
{
    unsigned int flags = DEFAULT_POLICY_FLAGS | STANDARD_SCRIPT_VERIFY_FLAGS; // TODO read again
...

    {
        ...
        CAmount nFees = 0;
        if (!Consensus::CheckTxInputs(tx, state, flags, view, GetSpendHeight(view), nFees, nSigOps))
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck)
{
    ...
    // CreateNewBlock invocations which don't have a hash
    unsigned int flags = GetConsensusFlags(block, chainparams.GetConsensus(), pindex, !pindex->phashBlock);
    ...
    {
        const CTransaction &tx = block.vtx[i];
        if (!Consensus::VerifyTx(tx, state, flags, nHeight, nMedianTimePast, block.nTime, fScriptChecks && !fScriptCheckThreads, 
                                 fCacheResults, pindex, view, nFees, nSigOps))
            return error("%s: Consensus::VerifyTx on %s: %s", __func__, tx.GetHash().ToString(), FormatStateMessage(state));

        nInputs += tx.vin.size();
        if (!tx.IsCoinBase() && fScriptCheckThreads) {
...
static bool AcceptBlockHeader(const CBlockHeader& block, CValidationState& state, const CChainParams& chainparams, CBlockIndex** ppindex=NULL)
{
    ...
    if (hash != chainparams.GetConsensus().hashGenesisBlock) {
        ...
        if (!Consensus::VerifyBlockHeader(block, state, chainparams.GetConsensus(), GetAdjustedTime(), pindexPrev))
...
static bool AcceptBlock(const CBlock& block, CValidationState& state, const CChainParams& chainparams, CBlockIndex** ppindex, bool fRequested, CDiskBlockPos* dbp)
{
    ...
    const int64_t nHeight = pindex->pprev == NULL ? 0 : pindex->pprev->nHeight + 1;
    unsigned int flags = GetConsensusFlags(block, chainparams.GetConsensus(), pindex->pprev, !pindex->pprev->phashBlock);
    if (!Consensus::CheckBlock(block, state, chainparams.GetConsensus(), GetAdjustedTime()) ||
        !Consensus::CheckTxCoinbase(block.vtx[0], state, flags, nHeight)) {
