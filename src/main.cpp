...
CTxMemPool mempool(globalPolicy);
                        ...
                        bool* pfMissingInputs, bool fRejectAbsurdFee)
{
...

    {
        ...
        if (fLimitFree && !policy.ApproveFeeRate(CFeeRate(nFees, nSize))) {
            double dNextBlockPriority = view.GetPriority(tx, chainActive.Height() + 1);
            if (!policy.ApproveFreeTx(nSize, state, dNextBlockPriority, pool.IsPrioritized(hash)))
                return error("%s: not enough fees %d (tx %s)", __func__, nFees, hash.ToString());
        }

        if (fRejectAbsurdFee && !policy.ApproveAbsurdFee(nFees, state, nSize))
            return false;
