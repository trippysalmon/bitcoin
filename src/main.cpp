...
bool CScriptCheck::operator()() const {
    ...
    if (!VerifyScript(scriptSig, scriptPubKey, nFlags, CachingSignatureChecker(TxSignatureHasher(*ptxTo, nIn), cache)))
        return error("CScriptCheck() : %s VerifySignature failed", ptxTo->GetHash().ToString());
    return true;
}

bool CheckInputs(const CTransaction& tx, CValidationState &state, const CCoinsViewCache &inputs, bool fScriptChecks, unsigned int flags, bool cache, std::vector<CScriptCheck> *pvChecks)
{
    if (!tx.IsCoinBase())
    {
        if (pvChecks)
        ...
        if (fScriptChecks) {
            for (unsigned int i = 0; i < tx.vin.size(); i++) {
                ...
                // Verify signature
                CScriptCheck check(*coins, tx, i, flags, cache);
                if (pvChecks) {
                    ...
                    if (flags & STANDARD_NOT_MANDATORY_VERIFY_FLAGS) {
                        ...
                        CScriptCheck check(*coins, tx, i,
                                flags & ~STANDARD_NOT_MANDATORY_VERIFY_FLAGS, cache);
                        if (check())
