                  ...
                  CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck)
{
    ...
    CBlockIndex *pindexBIP34height = pindex->pprev->GetAncestor(chainparams.GetConsensus().nOldDeploymentHeights[Consensus::BIP34_HEIGHT_ACTIVE]);
    ...
    if (pindex->nHeight >= chainparams.GetConsensus().nOldDeploymentHeights[Consensus::BIP66_HEIGHT_ACTIVE]) {
        flags |= SCRIPT_VERIFY_DERSIG;
    }

    // Start enforcing CHECKLOCKTIMEVERIFY (BIP65) rule
    if (pindex->nHeight >= chainparams.GetConsensus().nOldDeploymentHeights[Consensus::BIP65_HEIGHT_ACTIVE]) {
...
bool ContextualCheckBlockHeader(const CBlockHeader& block, CValidationState& state, const Consensus::Params& consensusParams, const CBlockIndex* pindexPrev, int64_t nAdjustedTime)
{
    ...
    if((block.nVersion < 2 && nHeight >= consensusParams.nOldDeploymentHeights[Consensus::BIP34_HEIGHT_ACTIVE]) ||
       (block.nVersion < 3 && nHeight >= consensusParams.nOldDeploymentHeights[Consensus::BIP66_HEIGHT_ACTIVE]) ||
       (block.nVersion < 4 && nHeight >= consensusParams.nOldDeploymentHeights[Consensus::BIP65_HEIGHT_ACTIVE]))
...
bool ContextualCheckBlock(const CBlock& block, CValidationState& state, const CBlockIndex* pindexPrev)
{
    ...
    if (nHeight >= consensusParams.nOldDeploymentHeights[Consensus::BIP34_HEIGHT_ACTIVE])
    {
