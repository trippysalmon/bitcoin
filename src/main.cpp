...
{
    if (state->pindexBestKnownBlock && pindex == GetAncestor(state->pindexBestKnownBlock, pindex->nHeight, GetPrevIndex, GetSkipIndex))
        return true;
    if (state->pindexBestHeaderSent && pindex == GetAncestor(state->pindexBestHeaderSent, pindex->nHeight, GetPrevIndex, GetSkipIndex))
        return true;
    return false;
}
...
CBlockIndex* LastCommonAncestor(CBlockIndex* pa, CBlockIndex* pb) {
    if (pa->nHeight > pb->nHeight) {
        pa = (CBlockIndex*)GetAncestor(pa, pb->nHeight, GetPrevIndex, GetSkipIndex);
    } else if (pb->nHeight > pa->nHeight) {
        pb = (CBlockIndex*)GetAncestor(pb, pa->nHeight, GetPrevIndex, GetSkipIndex);
    }
...
void FindNextBlocksToDownload(NodeId nodeid, unsigned int count, std::vector<CBlockIndex*>& vBlocks, NodeId& nodeStaller) {
    ...
    while (pindexWalk->nHeight < nMaxHeight) {
        ...
        pindexWalk = (CBlockIndex*)GetAncestor(state->pindexBestKnownBlock, pindexWalk->nHeight + nToFetch, GetPrevIndex, GetSkipIndex);
...
bool CheckFinalTx(const CTransaction &tx, int flags)
{
    ...
    const int64_t nBlockTime = (flags & LOCKTIME_MEDIAN_TIME_PAST)
                             ? GetMedianTimePast(chainActive.Tip(), consensusParams, GetPrevIndex)
                             : GetAdjustedTime();
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck)
{
    ...
    if (fCheckpointsEnabled) {
        ...
        if (pindexLastCheckpoint && GetAncestor(pindexLastCheckpoint, pindex->nHeight, GetPrevIndex, GetSkipIndex) == pindex) {
    ...
    CBlockIndex* pindexBIP34height = (CBlockIndex*)GetAncestor(pindex->pprev, chainparams.GetConsensus().BIP34Height, GetPrevIndex, GetSkipIndex);
    ...
    if (block.nVersion >= 4 && IsSuperMajority(4, pindex->pprev, chainparams.GetConsensus().nMajorityEnforceBlockUpgrade, chainparams.GetConsensus(), GetPrevIndex)) {
...
static bool ActivateBestChainStep(CValidationState& state, const CChainParams& chainparams, CBlockIndex* pindexMostWork, const CBlock* pblock)
{
    ...
    while (fContinue && nHeight != pindexMostWork->nHeight) {
        ...
        CBlockIndex* pindexIter = (CBlockIndex*)GetAncestor(pindexMostWork, nTargetHeight, GetPrevIndex, GetSkipIndex);
...
bool ReconsiderBlock(CValidationState& state, CBlockIndex *pindex) {
    ...
    BlockMap::iterator it = mapBlockIndex.begin();
    while (it != mapBlockIndex.end()) {
        if (!it->second->IsValid() && (CBlockIndex*)GetAncestor(it->second, nHeight, GetPrevIndex, GetSkipIndex) == pindex) {
...
static bool AcceptBlockHeader(const CBlockHeader& block, CValidationState& state, const CChainParams& chainparams, CBlockIndex** ppindex=NULL)
{
    ...
    if (hash != chainparams.GetConsensus().hashGenesisBlock) {
        ...
        if (!Consensus::ContextualCheckBlockHeader(block, state, chainparams.GetConsensus(), pindexPrev, GetPrevIndex, GetSkipIndex))
