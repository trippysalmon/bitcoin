...
CTxMemPool mempool(PolicyGlobal::minRelayTxFee);
...
CAmount GetMinRelayFee(const CTransaction& tx, unsigned int nBytes, bool fAllowFree)
{
    ...
    CAmount nMinFee = PolicyGlobal::minRelayTxFee.GetFee(nBytes);
                        ...
                        bool* pfMissingInputs, bool fRejectInsaneFee)
{
...

    {
        ...
        if (!tx.IsCoinBase() && !Consensus::CheckTxInputs(tx, state, view, nSpendHeight))
        ...
        if (fRejectInsaneFee && nFees > PolicyGlobal::minRelayTxFee.GetFee(nSize) * 10000)
            return error("AcceptToMemoryPool: insane fees %s, %d > %d",
                         hash.ToString(),
                         nFees, PolicyGlobal::minRelayTxFee.GetFee(nSize) * 10000);
...
/** Convenient constructor for CScriptCheck */
class CScriptCheckFromCoins : public CScriptCheck
{
public:
    CScriptCheckFromCoins(const CCoins& txFromIn, const CTransaction& txToIn, unsigned int nInIn, unsigned int nFlagsIn, bool cacheIn) :
        CScriptCheck(txFromIn.vout[txToIn.vin[nInIn].prevout.n].scriptPubKey, txToIn, nInIn, nFlagsIn, cacheIn) { }
};
    ...
    if (!tx.IsCoinBase())
    {
        ...
        // still computed and checked, and any change will be caught at the next checkpoint.
            for (unsigned int i = 0; i < tx.vin.size(); i++) {
                ...
                // Verify signature
                CScriptCheckFromCoins check(*coins, tx, i, flags, cacheStore);
                if (pvChecks) {
                    ...
                    if (flags & STANDARD_NOT_MANDATORY_VERIFY_FLAGS) {
                        ...
                        // non-upgraded nodes.
                        CScriptCheckFromCoins check(*coins, tx, i,
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck)
{
    ...
    for (unsigned int i = 0; i < block.vtx.size(); i++)
    {
        ...
        if (!tx.IsCoinBase())
        {
            // This check will be repeated in Consensus::CheckTxInputs, but scaring DoS 0 if fails instead of 100
            if (!view.HaveInputs(tx))
