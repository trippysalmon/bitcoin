                        ...
                        bool* pfMissingInputs, bool fRejectAbsurdFee)
{
...

    {
        ...
        if (!Consensus::CheckTxInputsScripts(tx, state, view, true))
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck)
{
    ...
    if (!Consensus::CheckBlock(block, GetAdjustedTime(), state, Params().GetConsensus(), !fJustCheck, !fJustCheck))
    ...
    if (Consensus::IsGenesisBlock(block.GetHash(), Params().GetConsensus())) {
        if (!fJustCheck)
    ...
    CAmount blockReward = Consensus::GetBlockValue(pindex->nHeight, Params().GetConsensus(), nFees);
...
int GetHeight(const CBlockIndex* pindexPrev)
{
...
bool AcceptBlockHeader(const CBlockHeader& block, CValidationState& state, CBlockIndex** ppindex)
{
    ...
    if (!Consensus::CheckBlockHeader(block, GetAdjustedTime(), state, Params().GetConsensus()))
        return error("%s: Consensus::CheckBlockHeader(): ", __func__, state.GetRejectReason().c_str());

    // Get prev block index
    CBlockIndex* pindexPrev = NULL;
    if (!Consensus::IsGenesisBlock(hash, Params().GetConsensus())) {
...
bool AcceptBlock(CBlock& block, CValidationState& state, CBlockIndex** ppindex, CDiskBlockPos* dbp)
{
    ...
    if (!Consensus::CheckBlock(block, GetAdjustedTime(), state, Params().GetConsensus()) || !Consensus::ContextualCheckBlock(block, state, pindex->pprev, GetHeight(pindex->pprev), Params().GetConsensus())) {
...
{
    // Preliminary checks
    bool checked = Consensus::CheckBlock(*pblock, GetAdjustedTime(), state, Params().GetConsensus());
...
bool TestBlockValidity(CValidationState &state, const CBlock& block, CBlockIndex * const pindexPrev, bool fCheckPOW, bool fCheckMerkleRoot)
{
    ...
    if (!Consensus::CheckBlock(block, GetAdjustedTime(), state, Params().GetConsensus(), fCheckPOW, fCheckMerkleRoot))
        return error("%s: Consensus::CheckBlock: %s", __func__, state.GetRejectReason().c_str());
    if (!Consensus::ContextualCheckBlock(block, state, pindexPrev, GetHeight(pindexPrev), Params().GetConsensus()))
...
bool CVerifyDB::VerifyDB(CCoinsView *coinsview, int nCheckLevel, int nCheckDepth)
{
    ...
    for (CBlockIndex* pindex = chainActive.Tip(); pindex && pindex->pprev; pindex = pindex->pprev)
    {
        ...
        if (nCheckLevel >= 1 && !Consensus::CheckBlock(block, GetAdjustedTime(), state, Params().GetConsensus()))
    ...
    try {
        ...
        while (!blkdat.eof()) {
            ...
            try {
                ...
                uint256 hash = block.GetHash();
                if (!Consensus::IsGenesisBlock(hash, Params().GetConsensus()) && mapBlockIndex.find(block.hashPrevBlock) == mapBlockIndex.end()) {
                ...
                if (mapBlockIndex.count(hash) == 0 || (mapBlockIndex[hash]->nStatus & BLOCK_HAVE_DATA) == 0) {
                    ...
                    if (state.IsError())
                        break;
                } else if (!Consensus::IsGenesisBlock(hash, Params().GetConsensus()) && mapBlockIndex[hash]->nHeight % 1000 == 0) {
